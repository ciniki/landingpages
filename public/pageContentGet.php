...
function ciniki_landingpages_pageContentGet($ciniki) {
    ...
    $rc = ciniki_landingpages_checkAccess($ciniki, $args['tnid'], 'ciniki.landingpages.pageContentGet');
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    }   
    ...
    if( $args['content_id'] == 0 ) {
        ...
        $rc = ciniki_core_sequencesNext($ciniki, $args['tnid'], 'ciniki.landingpages.item', 'page_id', $args['page_id']);
        if( $rc['stat'] != 'ok' ) {
            return $rc;
        }
    ...
    if( !isset($rc['content']) ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.landingpages.7', 'msg'=>'Content does not exist'));
    }
    if( isset($rc['content']) ) {
